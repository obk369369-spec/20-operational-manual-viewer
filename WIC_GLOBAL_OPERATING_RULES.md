...
- 장시간·예약·반복 실행이 필요하면 실제로 그런 기능을 지원하는 Work/자동화/외부 실행구조가 있을 때만 사용한다.
...
- `필요한 자료가 있습니까?`처럼 조사 책임을 고객에게 넘기는 질문은 금지한다.
...
`고객 식별 → 과거 이력 회수 → 공식 홈페이지/최신 공개자료 조사 → 담당업무 쉬운 해석 → 실제 판매 가능 보고서 조사 → 핵심/확장/안전 추천 → 유선 멘트 → 상대답변 분기 → 메일/문자 → 최종 행동 → 누락검사`
...
| 7 | 고객 컨택 판단 | 저장소명 07-wic-setting-tool-v1 CONNECTED지만 목적 불일치 | 고객 컨택 규칙은 본 마스터에 LOCK, 저장소 실행판은 HOLD, Chat-native 실행은 ACTIVE |
...
- 고객 컨택 7번 규칙: 핵심 규칙 VERIFIED / LOCK, GitHub 실행판 목적 불일치 HOLD, Chat-native 실행 ACTIVE
- 중간 안내서/1번 규칙: 과거 가짜 데이터 생성 규칙 발견 → DEPRECATED, 실제 데이터 전용으로 재잠금, 콘텐츠 확장 규칙 추가
