...
- 이 게이트는 **실제 파일·코드·규칙·연결 수정이 발생한 현재 대상 TOOL에만** 적용한다. 작업하지 않은 다른 TOOL의 조사·테스트·개발을 유발하지 않으며, 영향받지 않은 기존 PASS/VERIFIED는 `SKIP_REUSE`한다.
- 실제 수정이 발생한 WIC의 강제 순서는 `수정 → 영향받은 기존 기능 회귀검사 → 실제 업무 입력 E2E → 최종 출력 정상 검증 → 오류 수정 → 동일 실패 입력 재테스트 → PASS 후 GitHub 반영 → remote SHA/file read-back → 기존 로컬 실행폴더 배포 → 배포된 canonical 파일 자체 재테스트 → 완료`다.
- 배포 승인에는 `실제 테스트 실행 증거 / 테스트 입력 / 독립적으로 정의한 EXPECTED / 실행에서 캡처한 ACTUAL / EXPECTED와 ACTUAL의 항목별 일치 / 영향범위 회귀검증 / PASS evidence`가 각각 필요하다. 하나라도 없으면 `DEPLOY_BLOCKED`이며 commit/push·파일 복사·완료 판정을 시작하지 않는다.
- 파일 존재, 화면 열림, 버튼·handler 존재, 종료코드 0, 행 수만 일치, 파일 생성·다운로드, commit, hash 일치, 프로그램 자체 PASS 문구는 단독 기능 PASS 증거가 아니다.
- 테스트 범위는 수정 영향범위에 비례한다. 작은 UI·필드·서식 변경은 해당 기능과 직접 영향 경로를 대표 입력으로 검증하고, 영향받지 않은 대량처리·기존 기능은 `SKIP_REUSE`한다. 공통·대량 엔진 변경 또는 기존 PASS와 충돌하는 실제 증거가 있을 때만 범위를 확대한다.
- GitHub/read-back 이후 로컬 canonical 배포 또는 배포본 재테스트가 빠지면 현재 작업을 `DEPLOY_INCOMPLETE`로 유지하고 `DEPLOY_LOCAL_CANONICAL_AND_RETEST`를 자동 다음 단계로 실행한다. 별도 사용자 재지시를 기다리지 않으며, 권한·로그인·MFA처럼 Work가 해제할 수 없는 차단만 `USER_ACTION_REQUIRED`로 남긴다.
