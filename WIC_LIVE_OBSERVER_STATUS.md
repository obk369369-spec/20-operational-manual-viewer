...
## 대화창 계보 / 이름 관리
- 기준 부모 대화창: `워크 작업 전 각 도구 준비`
- GitHub 관리명 규칙: 새/파생 작업 세션은 항상 부모 이름을 앞에 이어서 기록한다.
- 현재 작업 세션 GitHub 관리명: `워크 작업 전 각 도구 준비 → WIC observer-mode 고객응대 루틴`
...
- 따라서 ChatGPT가 작업 과정에서 새 세션/대화를 자동 생성했는지, 제품 UI가 자동 제목을 붙였는지는 별도 제품 동작이며 GitHub에서는 계보를 명시적으로 보존한다.
- 사용자가 사이드바 대화창 이름을 변경한 사실은 GitHub가 자동 감지하지 못한다. 사용자가 현재 메시지에서 알려준 명칭 또는 실제 접근 가능한 대화 메타데이터가 있을 때만 반영한다.
...
- assistant는 현재 시스템에서 사용자 Chat 목록에 새 대화창을 직접 생성하거나 이름 붙일 수 있다고 확인되지 않는다. 과거 파일명에 `새 대화창에서 이어서 진행`이 있어도 그것은 과거 대화 기록/파일 제목이며, 이번 observer 작업이 새 Chat 창을 직접 생성했다는 뜻은 아니다.
