...
## 7. 과거 규칙 재검증 결과 — 새 규칙 생성보다 기존 규칙 강제 실행
과거 사용자가 제공한 범용 지시문에는 이미 다음 핵심 규칙이 존재했다.
- 고정 거래 발행사 풀 밖 발행사 사용 금지.
- 최소 3개 이상의 서로 다른 발행사 사용 및 동일 발행사 반복 금지.
- 각 발행사 내 최신 발행일 우선, 오래된 자료 제외.
- 기관 맥락 → 부서 기능 → 담당업무 → 사용자 원문 순으로 직접 연결되는 자료만 선택.
- `제목 = 링크 페이지 제목 완전 일치`, 링크 필수, 불일치 시 폐기.
- 안내서 기본 정보는 발행사/페이지/가격/발행일/링크와 상위·하위 목차를 고정 구조로 사용.
따라서 최근 불안정성의 원인은 규칙 부족 자체가 아니라, 출력 직전에 기존 규칙을 항목별로 강제 확인하는 실행 게이트가 빠진 데 있다.

## 8. 안내서 출력 전 필수 PRE-OUTPUT GATE — 하나라도 실패하면 고객용 출력 금지
자료 1~3 각각 아래 10개 항목을 모두 확인한 뒤에만 출력한다.
...
8. TOC_PASS: 공식 상세페이지에서 목차가 확인되며 상위+하위까지만 원문 순서대로 추출했는가.
9. INFO_PASS: 보고서 정보가 공식 페이지 원문을 한국어로 번역만 한 것이며 분석/요약/재구성이 없는가.
10. CLEAN_PASS: `utm_source=chatgpt.com`, Markdown 강조기호, AI 작성 흔적, 검색 흔적이 없는가.

- 자료 1~3 중 하나라도 위 10개 중 하나가 FAIL이면 3종 전체를 완료로 출력하지 않는다.
- FAIL 자료만 교체하거나 재검증한 뒤 다시 10개 게이트를 통과시킨다.
- `검색결과에서 봤다`, `메타데이터에 있었다`, `비슷한 보고서에 가격이 있었다`는 PASS 근거가 아니다.
- 값의 출처 위치를 내부적으로 `TITLE/H1`, `PRICE/PURCHASE OPTIONS`, `DATE`, `PAGES`, `TOC`, `DESCRIPTION` 단위로 기록한다.
- 사용자에게 보여주는 최종 안내서에는 내부 검증표를 붙이지 않되, 사용자가 질문하면 어느 위치에서 확인했는지 즉시 설명할 수 있어야 한다.

## 9. 제목 판정 우선순위
고객용 제목 판정은 아래 순서로 한다.
1. 상세페이지에서 실제 보고서명으로 크게 표시된 H1/대표 제목.
2. 해당 페이지의 명시적 Report Title/보고서명 블록.
3. 위 둘이 없는 경우에만 공식 페이지 내 동일 보고서 식별용 제목.
