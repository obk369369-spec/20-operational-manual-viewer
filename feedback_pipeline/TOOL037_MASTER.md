...
## 9. 비번호/별칭 역사자료 흡수 — Batch 1
대상: `[34번 공통 보완 추가 — 37번 메타데이터 전 자료형 통합·자동 검증·규칙 잠금 운영 V1.0].txt`.

분류: `ALIAS_TO_TOOL037 / ABSORBED_DIFF_ONLY`.

추가로 보존하는 정상 DIFF:
- 샘플 몇 행만 맞는 것으로 완료 처리하지 않는다. 변경된 변환 범위에서는 첫 행·중간 행·마지막 행·빈값/가격/카테고리/날짜/페이지/URL/긴 개요·목차 등 대표 위험행을 포함해 검증한다.
- 원본 행수와 출력 행수 일치를 검사하고, 일부 행만 처리된 결과를 완료로 보지 않는다.
- 홈페이지 최종 컬럼 수·명칭·순서, 발행사 컬럼, 카테고리 빈칸, 날짜형식, 가격 대상 컬럼, 통화, URL 위치, 개요·목차 누락, 불필요 공백, 중복 보고서 등 실제 출력값을 확인한다.
- `- 월드입력` 파일명 규칙은 최종 출력 명명 규칙으로 유지한다. `최종`, `V1`, `수정본` 같은 임의 버전명 증식을 기본 운영으로 사용하지 않는다.
- 새 발행사 수정사항을 전체 발행사에 자동 전파하지 않는다. 현재 GLOBAL의 no-full-audit 규칙에 따라 실제 영향 범위만 좁게 판정한다.
- 과거 문서가 요구한 다중 규칙관리 파일 동기화 구조는 현재 CENTRAL 단일 정본 원칙과 충돌할 수 있으므로 그대로 복제하지 않는다. 실제 활성 파일이 확인되면 정본/포인터 관계만 연결한다.
