...
def run():
    with tempfile.TemporaryDirectory() as td:
        ...
        (root/"evidence"/"tool016_history_coverage_audit_20260909.json").write_bytes((HERE/"evidence"/"tool016_history_coverage_audit_20260909.json").read_bytes())
        ...
        inferred=ingest({"source_chat_or_tool":"ANY_WIC_CHAT","user_original_text":"6번 목차 계층 오류가 또 발생했다.","timestamp":"2026-09-09T00:04:00Z"},root)
        ...
        assert inferred["tool_id"]=="TOOL006" and inferred["tool_inference"]=="INFERRED"
        assert len(ledger["roots"])==4 and snap["latest_feedback"][0]["root_id"]==inferred["root_id"]
        for result in (a,direct,other,inferred): assert any(d.get("request_id")==result["tool044_demand_id"] for d in queue["demands"])
        assert snap["truth_boundary"]["any_chat_auto_access"]=="ACCESS_NOT_AVAILABLE"
        assert snap["history_coverage"]["full_history_coverage"]=="UNKNOWN"
        assert snap["parallel_status"]["active_workers"]==0
        return {"status":"PASS","tests":{"tool_a_to_016_to_044":"PASS","tool_b_to_016_to_044":"PASS","tool044_direct_loop":"PASS","root_dedup_occurrence":"PASS","free_text_tool_inference":"PASS","observer_coverage_readback":"PASS","parallel_truth_boundary":"PASS","unavailable_chat_truth_boundary":"PASS"},"roots":4,"requests":4}
