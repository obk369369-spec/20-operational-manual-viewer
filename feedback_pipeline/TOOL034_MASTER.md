...
## 9. 비번호/별칭 역사자료 분류 — Batch 1
아래 자료는 별도 TOOL/MASTER를 만들지 않고 34번 기준관리 역사 alias로 분류한다. 내용은 GLOBAL/CENTRAL의 기존 관찰자·껍데기 차단·외부검증·승인/실동작 증거 규칙과 대조했으며, 이 Batch에서는 별도 고유 DIFF를 추가하지 않는다.

- `[34번 공통 보완 추가 — 외부툴 설치·연결·실동작 100% 충족 및 유료 전환 준비 최종 잠금].txt`
- `[34번 공통 보완 추가 — 외부툴 설치·연결·실동작 100% 충족 및 유료 전환 준비 최종 잠금](1).txt`
- `[34번 공통 보완 추가 — 외부툴 설치·연결·실동작 100% 충족 및 유료 전환 준비 최종 잠금](2).txt`
- `[34번 공통 보완 추가 — 외부툴 설치·연결·실동작 100% 충족 및 유료 전환 준비 최종 잠금](3).txt`
- `WIC34 외부툴 협업 전환 보완 잠금.txt`
- `[34번 공통 보완 추가 — 외부툴 검증체계 실동작 잠금].txt`
- `34번 공통 보완 추가 — Connection AI ,내부 에이전트 10개 역할배치·외부툴 교차검증·야간감시·사무실 이동패키지 최종 잠금(1).txt`
- `34번 공통 보완 추가 — Connection AI ,내부 에이전트 10개 역할배치·외부툴 교차검증·야간감시·사무실 이동패키지 최종 잠금(2).txt`

분류: `ALIAS_TO_TOOL034 / SKIP_REUSE_NO_UNIQUE_DIFF`
- 파일명·버전이 다르다는 이유로 별도 정본을 만들지 않는다.
- 과거 외부툴 설치/검증/야간감시 문구 자체를 현재 실동작 증거로 승격하지 않는다.
- 현재 실제 Work에서 관련 USB/외부자산을 만나면 공통 USB 검증 계약에 따라 다시 변경범위만 판정한다.

## 10. 비번호/별칭 역사자료 분류 — Batch 2
역사자료 `34번 통합+자동화 에이전트 지시문 저장.doc`, `(1).doc`, `(2).doc` 및 같은 계열의 `34번 통합+자동화 2` 사본군은 34번 기준관리 역할을 만들던 이전 세대 자료로 분류한다.

- 문서에는 관찰자 모드, 반복 전달 차단, 자동 인계, STOP/PASS 판정, 기준문서 통합 등의 방향이 있으나 현재 GLOBAL/CENTRAL과 본 MASTER에 이미 흡수된 내용은 `SKIP_REUSE`한다.
- 과거 문서가 스스로 `최종 기준본`, `완전본`, `잠금판`이라고 부른 것만으로 현재 정본 지위를 부여하지 않는다.
- 과거 기록 자체가 긴 본문 절단·축약·부분 출력 및 누락 가능성을 인정하므로 완전 무누락 원본으로 취급하지 않는다.
- 과거의 `전수 점검` 지시는 현재 `FULL_AUDIT_BLOCKED` 규칙에 따라 재실행하지 않는다.
- 과거의 실행도구/자동화 에이전트/스마트폰 런처 연결 주장은 당시 기록에서도 미완료·PASS 아님으로 남아 있으므로 현재 실동작 증거로 승격하지 않는다.
- `34번 통합+자동화 2`의 여러 번호 사본은 동일 묶음/파일지도 반복본 성격이 확인되어 별도 MASTER를 만들지 않는다.

분류: `ALIAS_TO_TOOL034 / HISTORICAL_PREDECESSOR / SKIP_REUSE_NO_UNIQUE_DIFF`.
