...
with tempfile.TemporaryDirectory() as directory:
    ...
    assert first["demands_processed"] == 8
    ...
    # Only unresolved capabilities are backoff-blocked. Capabilities verified
    # in the registry are reused immediately and are never searched again.
    assert second["duplicate_searches_blocked"] == 4
    second_by_id = {item["demand_id"]: item for item in second["results"]}
    assert second_by_id["TOOL044-FREE-LOCAL-EXECUTION-ENGINE"]["result"] == "READY_ATOMIC_COMPONENT_FOUND"
...
print(json.dumps({"status":"PASS","demands":8,"duplicate_searches_blocked":4,"cost":0}))
